...
const { getARAmount } = require('../../../../app/reports/summary/get-ar-amount')
...
describe('get AR amount', () => {
  beforeEach(() => {
    jest.clearAllMocks()
    events = [extractedEvent, ev1]
  })

  test('should return the correct total from the processed event with AR ledger', () => {
    const amount = getARAmount(events)
    expect(amount).toEqual(-ev1.data.value)
  })

  test('should return correct total if multiple processed event with AR ledger', () => {
    events.push(ev2)
    const amount = getARAmount(events)
    expect(amount).toEqual(-ev1.data.value - ev2.data.value)
  })

  test('should ignore processed events with AP ledger', () => {
    events.push(ev3)
    const amount = getARAmount(events)
    expect(amount).toEqual(-ev1.data.value)
  })
